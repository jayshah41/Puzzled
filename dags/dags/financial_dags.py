...
def financial_data():
    ...
    # Include all Periods data 
    fin_df = fin_df.withColumn("Period", regexp_replace(fin_df["Period"], "FY$", "AN"))
    fin_df = fin_df.filter(~col("Period").contains("NA"))
    ...
    m_df1=m_df.select('ASX Codes','Project Stage','Project Location Continent','Project Location State', 'Drilling Periods')
    m_df2=m_df1.toPandas()
    pdfs=[]
    for i in m_df2.groupby(['ASX Codes', 'Project Location Continent']):
        pdf=i[1][['Project Stage','Project Location Continent','ASX Codes','Project Location State', 'Drilling Periods']].iloc[0:1]
        pdfs.append(pdf.to_dict('records')[0])
    df = pd.DataFrame(pdfs)
    sparkDF=spark.createDataFrame(df) 
    ...
    m_df1=c_df.select("ASXCODE", "Company")
    m_df2=m_df1.toPandas()
    pdfs=[]
    for i in m_df2.groupby('ASXCODE'):
        pdf=i[1][["ASXCODE", "Company"]].iloc[0:1]
        pdfs.append(pdf.to_dict('records')[0])
    df = pd.DataFrame(pdfs)
    sparkDF=spark.createDataFrame(df) 
    sparkDF=sparkDF.selectExpr("ASXCODE", "Company")
    spark_fil_new=sparkDF.join(spark_fil,sparkDF.ASXCODE  ==  spark_fil.ASX,"left")

    # Aggregate multiple values into array of strings

    spark_fil1=spark_fil_new.withColumn('con', F.explode(F.array('Project Stage'))).withColumn('con1', F.explode(F.array('Project Location Continent'))).withColumn('con2', F.explode(F.array('Project Location State'))).withColumn('con3', F.explode(F.array('Drilling Periods')))\
            .groupby(
            'EditDate',
            'ASXCodes',
            'Period',
            ...
            ).agg(F.collect_set('con').alias('Project Stage'),F.collect_set('con1').alias('Project Location Continent'),F.collect_set('con2').alias('Project Location State'), F.collect_set('con3').alias('Drilling Periods'))
            

    spark_fil1.withColumn("Priority Commodities",priorityCommodities_udf(col("Priority Commodities"))).write.format("org.elasticsearch.spark.sql").option("es.resource", '%s' % ('asx_financials_tier1')).option("es.net.http.auth.user", "elastic").option("es.net.http.auth.pass", "changeme").option("es.net.ssl", "true").option("es.nodes","http://54.66.84.237").option("es.port", "9200").option("es.mapping.rich.date", "false").option("es.nodes.wan.only","true").mode("overwrite").save()
