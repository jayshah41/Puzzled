...
describe('NewsContent Component', () => {
  ...
  test('handles apiRequest error when fetch throws', async () => {
    console.error.mockClear();
  ...
  test('renders differently based on isLoggedIn flag', async () => {
    localStorageMock.getItem.mockImplementation((key) => {
      if (key === 'user_tier_level') return null;
      if (key === 'accessToken') return 'mock-token';
      return null;
    ...
    expect(container.querySelector('.news-title')).toBeTruthy();
    expect(container.querySelector('.edit-button')).toBeFalsy();
  });
  
  test('increases coverage for non-JSON response handling', async () => {
    localStorageMock.getItem.mockImplementation((key) => {
      if (key === 'user_tier_level') return "2";
      if (key === 'accessToken') return 'mock-token';
      return null;
  ...
  test('validates missing paragraphs in cards', async () => {
    localStorageMock.getItem.mockImplementation((key) => {
      if (key === 'user_tier_level') return "2";
      if (key === 'accessToken') return 'mock-token';
      return null;
    ...
    global.fetch.mockResolvedValueOnce({
      ok: true,
      status: 200,
      json: async () => ([{ id: 1, title: 'Test', category: 'Test', date: 'Test', link: 'https://example.com', order: 0 }]),
      headers: { get: () => 'application/json' }
    });
