...
const CompanyDetails = () => {
  ...
  const metrics = [
    {
      ...
      value: new Set(companyData.map(d => d.asx_code)).size
    },
    {
      title: "Total Companies",
      value: new Set(companyData.map(d => d.company_name)).size
    },
    {
      title: "Project Areas",
      value: new Set(companyData.map(d => d.project_area).filter(Boolean)).size
    }
  ];
  ...
  const getUnique = (key) =>
    [...new Set(companyData.map(item => item[key]).filter(Boolean))].map(value => ({
      label: value,
      value
    }));
      ...
      {loading ? (
        ...
        <GraphPage
          ...
          chartData={[
            ...
            { title: "Top 5 Values by Project Area", type: "bar", data: valueByProjectArea() }
          ]}
